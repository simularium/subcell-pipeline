...
for repeat, df_repeat in tomogram_df.groupby("unique_identifier"):
    ...
    df_tmp["velocity"] = 0
    df_tmp["repeat"] = repeat
    df_tmp["X"], df_tmp["Y"], df_tmp["Z"]  = df_repeat["X"].values, df_repeat["Y"].values, df_repeat["Z"].values
...
# %%
chosen_metrics = [COMPRESSIONMETRIC.CONTOUR_LENGTH, COMPRESSIONMETRIC.AVERAGE_PERP_DISTANCE,COMPRESSIONMETRIC.CALC_BENDING_ENERGY, 
                        COMPRESSIONMETRIC.NON_COPLANARITY, COMPRESSIONMETRIC.PEAK_ASYMMETRY, COMPRESSIONMETRIC.TOTAL_FIBER_TWIST, COMPRESSIONMETRIC.COMPRESSION_RATIO]
