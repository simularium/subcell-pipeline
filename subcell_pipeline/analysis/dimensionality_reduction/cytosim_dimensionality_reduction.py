...
def figure_filename(strs: List[str]) -> str:
    ...
    return (
        dir_figures
        + snake_case(today_str)
        + "/"
        + snake_case(today_str + "_" + "_".join(strs))
        + ".png"
    )
...
def color_fader(c1, c2, mix=0) -> str:
    ...
    return mpl.colors.to_rgb(
        (1 - mix) * c1 + mix * c2
    )  # dropping to rgb because alpha is being used by metrics now


def color_list_generator(
    num_of_vals_to_make: int,
    idx: int = 0,
    c1_override: str = None,
    c2_override: str = None,
) -> list[str]:
    ...
    return [
        color_fader(c1, c2, i / num_of_vals_to_make) for i in range(num_of_vals_to_make)
    ]
...
def calculate_fiber_rotation(fiber: np.ndarray) -> float:
    ...
    angle_rad = np.arctan2(
        fiber[max_distance_monomer_index, 2], fiber[max_distance_monomer_index, 1]
    )
...
def align_fibers(fibers: np.ndarray) -> np.ndarray:
    ...
    for fiber_idx, fiber in enumerate(fibers.copy()):
        ...
        for fiber_timepoint_idx, fiber_timepoint in enumerate(fiber):
            ...
            fiber_rotated[:, 1:] = np.dot(
                fiber_timepoint_shaped[:, 1:], rotation_matrix
            )
            ...
            for repeat, repeat_df in velocity_df.groupby("repeat"):
                ...
                for time, time_df in repeat_df.groupby("time"):
                    if len(fiber_timepoints) >= num_timepoints:
                        break
                    fiber_timepoints.append(
                        time_df[["xpos", "ypos", "zpos"]].values.flatten()
                    )  # flattening to match data patterns
                    # HACK: we should do this all w/ dataframes but the refactor is painful so going to try keeping metrics in parallel
                    try:
                        fiber_timepoints_metrics.append(
                            time_df[repeat_time_metrics_list]
                        )
                    except Exception as err:
                        pass  # if metrics don't exist, the append errors but just let things continue
                fibers.append(
                    fiber_timepoints
                )  # 1001 timepoints x flattened(50 monomers x 3 dimensions)
            ...
            df = pd.DataFrame(
                {
                    "fibers": [
                        np.array(fibers)
                    ],  # need as np array for shape property, should be (-1, 600)
...
def prep_study_df_fibers(study_df: pd.DataFrame, align: bool = False):
    ...
    # align or dont
    fibers = (
        study_df.fibers[0].copy()
        if align == False
        else align_fibers(fibers=study_df.fibers[0])
    )
    # normalize scale across simulators
    fibers_scaled = (
        fibers * study_df.param_coordinate_scaler[0]
    )  # scale the coordinates for aligning multiple sims
    # reshape
...
# %%
# ### PCA: Setup
def get_study_pca_dfs(
    study_dfs: List[pd.DataFrame], align: bool
) -> [List[pd.DataFrame], PCA]:
    """
    ...
    combined_prepared_data = np.vstack(
        study_fibers
    )  # needing to create a 1 dim arr, but errs if I just pass in [study_fibers], TODO: better understand this
...
def plot_study_dfs(
    ...
    pca_space: PCA,
    study_dfs: pd.DataFrame,
    title: str,
    figsize: int = 8,
    alpha_by: str = None,
    color_by: str = None,
    skip_metrics: bool = False,
    only_plot_source: str = None,
):
    """
    ...
    for metric in repeat_time_metrics_list:
        ...
        # --- setup fig
        fig, ax = plt.subplots(
            figsize=(figsize, figsize) if type(figsize) == int else figsize
        )
        # ax.set_xlim(-0.9, 0.9)
        # ax.set_ylim(-0.9, 0.9)
        # --- List of parameters or conditions under which the PCA was run.
        ax.set_xlabel(
            f"PC1{f': {pca_space.explained_variance_ratio_[0]}' if pca_space != None else ''}",
            loc="left",
        )
        ax.set_ylabel(
            f"PC2{f': {pca_space.explained_variance_ratio_[1]}' if pca_space != None else ''}",
            loc="bottom",
        )
        ...
        for analysis_idx, analysis_df in enumerate(analysis_dfs):
            ...
            num_timepoints = analysis_df.time.iloc[
                -1
            ]  # this is a series. each monomer point is saved with a time position. we grab the last value to know what the last monomer timepoint is
            # --- flattened metrics (bc points are flattened when processed ex: 5 fiber x 200 timepoint becomes 1000 points)
            flattened_metrics = [
                repeat_time[metric].values[0]
                for repeat_list in study_dfs[analysis_idx].fibers_metrics[0]
                for repeat_time in repeat_list
            ]
            ...
            for fiber_idx in range(num_pc1_points // num_timepoints):
                if metric == "time":
                    pc1_color_lists.append(
                        color_list_generator(num_timepoints, analysis_idx)
                    )  # all fibers for this sim will be same color, but we'll have opacity shift
                    pc1_marker_lists.append(
                        marker_list_generator(num_timepoints, fiber_idx)
                    )  # for each fiber, fill a list of symbol tags
                else:
                    pc1_color_lists.append(
                        color_list_generator(
                            num_timepoints,
                            analysis_idx,
                            c1_override="black" if metric != "time" else None,
                            c2_override="black" if metric != "time" else None,
                        )
                    )  # all fibers for this sim will be same color, but we'll have opacity shift
                    pc1_marker_lists.append(
                        marker_list_generator(num_timepoints, analysis_idx)
                    )  # for each fiber, fill a list of symbol tags
            ...
            for i in range(num_pc1_points):
                ...
                alpha = (
                    normalize(flattened_metrics[i], metric_min, metric_max)
                    if len(flattened_metrics) > 0
                    else 0.8
                )
                ...
                if i == 0:
                    if metric == "time":
                        ...
                        legend_handles.append(
                            mpatches.Patch(color=color[0], label=label)
                        )
                    else:
                        label = f"{analysis_df.source[0]}/{analysis_df.param_velocity[0]} [{metric}]"
                        legend_handles.append(
                            mpatches.Patch(
                                color=color[0] if color_by == "source" else "white",
                                label=label,
                            )
                        )
                ax.scatter(
        ...
        )  # using handles so we can control how many legend items appear
...
def calc_pca_component_distributions(
    pca_space: PCA, pca_transform_dataframes: List[pd.DataFrame]
) -> List[dict]:
    ...
    for pca_transform_dataframe in pca_transform_dataframes:
        for idx, label in enumerate(["principal component 1", "principal component 2"]):
            ...
            for idx, val in enumerate(df["values"]):
                projected_fiber = pca_space.inverse_transform(
                    [val["input"][0], val["input"][1]]
                )  # duplicative, but just want to make clear the interface
                df["values"][idx]["fiber"] = projected_fiber.reshape(
                    -1, 3
                )  # transformation is a flat
            # --- append to set
            pca_sets.append(df)
    return pca_sets


def plot_inverse_transform_pca(
    pca_sets: List[pd.DataFrame],
    title_prefix: str,
    color_by: str = None,
        ...
        for idx_pca, pca_set in enumerate(pca_sets):
            for idx_val, val in enumerate(pca_set["values"]):
                ...
                axs[idx_pca % 2, idx_val].plot(
                    ...
                    c="g",
                    alpha=(1 / (idx_pca + 1)),
                    label=f"[{val['source']}] Fiber Z",
                )
                axs[idx_pca % 2, idx_val].set_xlabel("x")
                axs[idx_pca % 2, idx_val].set_ylabel("y")
                # prefix title for sim name / velocity params
                axs[idx_pca % 2, idx_val].set_title(
                    f"[{title_prefix}] PC1={str(val['input'][0])[0:6]} PC2={str(val['input'][1])[0:6]}",
                    fontsize=14,
                )
    ...
    if include_3d != False:
        ...
        for angle in [90, 60, 30, 0]:
            ...
            pca_sets_by_comp = [
                list(
                    filter(
                        lambda pca_set: pca_set["label"][0] == "principal component 1",
                        pca_sets,
                    )
                ),
                list(
                    filter(
                        lambda pca_set: pca_set["label"][0] == "principal component 2",
                        pca_sets,
                    )
                ),
            ]
            for idx_pca_comp, pca_set_by_component in enumerate(pca_sets_by_comp):
                ...
                ax.set_zlabel("Z")
                ax.set_title(
                    f"[{title_prefix},Deg={angle}] {pca_set_by_component[0]['label'][0]}",
                    fontsize=14,
                )
                legend_handles = []
                legend_labels = []
                for idx_pca_val, pca_set_val in enumerate(pca_set_by_component):
                    for idx_inverse_val, inverse_val in enumerate(
                        pca_set_val["values"]
                    ):
                        c = color_list[idx_inverse_val % len(color_list)]
                        x, y, z = zip(
                            *inverse_val["fiber"]
                        )  # TIL you can pass in a tuple of values, and this resolves the legend issue + visualization
                        if color_by == "source":
                        ...
                        if color_by == "param_velocity":
                            c = color_list[
                                velocity_to_idx(str(inverse_val["param_velocity"]))
                            ]
            ...
            plt.tight_layout()
            plt.savefig(
                figure_filename(["inverse_transform_3d", title_prefix, f"{angle}deg"])
            )
            plt.show()
...
def plot_subsample_fibers_dfs(study_dfs: List[pd.DataFrame], align: bool = False):
    ...
    for study_df in study_dfs:
        ...
        study_df_fibers = (
            align_fibers(fibers=study_df_fibers_slice)
            if align == True
            else study_df_fibers_slice
        )
        for fiber_idx, fiber in enumerate(study_df_fibers):
            # if fiber_idx > 1: continue
            for idx_fiber_timepoint, fiber_timepoint in enumerate(
                fiber.reshape(-1, 200, 3)
            ):  # seems to have 66 loops at 200/3
                # if idx_fiber_timepoint > 20: continue
                x, y, z = zip(*fiber_timepoint.reshape(-1, 3))
                plot.append(
                    [x, y, z, 0.6, color_list[color_increment % len(color_list)]]
                )
            color_increment += 1
    # PLOT
    for proj_type in ["persp", "ortho"]:
        ...
        for idx_angle, angle in enumerate([0, 90]):
            ...
            ax.view_init(
                elev=0 if proj_type == "ortho" else 20,
                azim=angle if proj_type == "ortho" else 45 + angle,
            )
        plt.tight_layout()
        plt.savefig(
            figure_filename(
                [
                    "subsample_fibers_3d",
                    f"aligned={align}" f"proj_type={proj_type}",
                    f"{angle}deg",
                ]
            )
        )
    ...
    for source_idx, source in enumerate(sources):
        ...
        for pc in ["principal component 1", "principal component 2"]:
            ...
            axs[idx_plt].hist(
                ...
                label=labels,
                color=(
                    color_list[source_idx] if len(dataset_midpoints) == 1 else None
                ),  # if many datasets combined, let it auto do colors
                alpha=1 if pc == "principal component 1" else 0.65,
            )  # doing this to make it visually easy to see PC1/PC2
...
for align in [True]:
    # --- pca
    pca_aligned_dfs_all, pca_aligned_space_all = get_study_pca_dfs(
        study_dfs, align=align
    )
    # --- inverse transform: min/maxs
    pca_distribution_df = get_pca_distribution_df(pca_aligned_dfs_all)
    pca_component_dists_standards = calc_pca_component_distributions(
        pca_space=pca_aligned_space_all, pca_transform_dataframes=pca_distribution_df
    )
    plot_inverse_transform_pca(
        pca_sets=pca_component_dists_standards,
        title_prefix=f"ALL: Aligned={align}",
        color_by="distribution",
    )
    ...
    # --- pca: plot
    plot_study_dfs(
        analysis_dfs=pca_aligned_dfs_all,
        pca_space=pca_aligned_space_all,
        study_dfs=study_dfs,
        title=f"ALL (PCA): Aligned={align},Velocity=All",
        figsize=(11, 11),
        color_by="source",
        skip_metrics=False,
    )
    plot_study_dfs(
        ...
        pca_space=pca_aligned_space_all,
        study_dfs=study_dfs,
        title=f"cytosim (PCA): Aligned=True,Velocity=All",
        figsize=(11, 11),
        color_by="source",
        alpha_by="param_velocity",
        skip_metrics=True,
        only_plot_source="cytosim",
    )
    plot_study_dfs(
        analysis_dfs=pca_aligned_dfs_all,
        pca_space=pca_aligned_space_all,
        study_dfs=study_dfs,
        title=f"readdy (PCA): Aligned=True,Velocity=All",
        figsize=(11, 11),
        color_by="source",
        alpha_by="param_velocity",
        skip_metrics=True,
        only_plot_source="readdy",
    )
    # --- histograms
