...
def download_data(conditions, num_repeats):
    ...
    for repeat in range(num_repeats):
        download_h5_file(f"actin_compression_baseline_{repeat}_0")
        for condition in conditions:
            download_h5_file(f"actin_compression_velocity={condition}_{repeat}")
...
def upload_to_s3(conditions, num_repeats):
    for repeat in range(num_repeats):
        upload_simularium_file(f"actin_compression_baseline_{repeat}_0")
        for condition in conditions:
            upload_simularium_file(f"actin_compression_velocity={condition}_{repeat}")
